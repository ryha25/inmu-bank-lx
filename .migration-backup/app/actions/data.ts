...
export async function getDashboard() {
  let userId: string
  let p: Awaited<ReturnType<typeof ensureProfile>>
  ...
  // Each query wrapped individually so one missing table doesn't kill everything
  let monthlyChange = 0
  let recent = EMPTY_DASHBOARD.recent
  try {
    ...
    // transactions table missing — leave defaults
  }

  let jarTotal = 0
  try {
    const jarRows = await db.select().from(jars).where(eq(jars.userId, userId))
    jarTotal = jarRows.reduce((s, j) => s + Number(j.balance), 0)
  } catch {
    // jars table missing — leave 0
  }
  ...
  try {
    ...
    // goals table missing — leave 0
  }

  // points table may not exist in production yet
  ...
  try {
    ...
    monthlyPointsTotal = monthlyPoints[0]?.total ?? 0
  } catch {
    // points table missing — leave 0
  }
  ...
  return {
    ...
    monthlyPoints: monthlyPointsTotal,
    monthlyChange,
    jarTotal,
    goalRate,
    recent,
...
export async function getCommunityStats() {
  try {
    ...
    return { participations: 0, receiveCount: 0, totalReceivedInmu: 0, rank: 1, totalUsers: 1 }
  }
}
...
export async function getPoints() {
  try {
    ...
    return { monthlyTotal: 0, allPoints: [] }
  }
}
...
export async function claimDailyLogin() {
  try {
    ...
    // Tables not yet migrated — return graceful no-op
    return { alreadyClaimed: true, bonus: 0, streak: 0 }
  }
}
