...
export function DashboardPage() {
  ...
  if (!data) return <AppShell isAdmin={profile?.role === 'admin'} displayName={profile?.displayName ?? ''} unread={unread}><div className="py-20 text-center text-muted-foreground">{t('loading')}</div></AppShell>

  return (
    <AppShell isAdmin={profile?.role === 'admin'} displayName={profile?.displayName ?? ''} unread={unread}>
      <PageHeader titleKey="nav_dashboard" />
      <DashboardView data={data} displayName={profile?.displayName ?? ''} />
    </AppShell>
  )
}
